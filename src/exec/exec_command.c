...
/*   Updated: 2025/03/17 12:25:37 by akyoshid         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../inc/pipex.h"

static int	_exec_command_fork_err(char *executable_path)
{
...
static void	_exec_command_execve_err(void)
{
...
int	exec_command(t_ast *node, t_data *data)
{
	...
	pid = fork();
	if (pid == -1)
		return (_exec_command_fork_err(executable_path));
	else if (pid == 0)
	{
		execve(executable_path, node->argv, data->envp);
		_exec_command_execve_err();
	}
